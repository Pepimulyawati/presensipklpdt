<template>
  <DashboardLayout>
    <div class="space-y-6">
      <div class="bg-white p-5 rounded-2xl border border-sky-100 shadow-2xs">
        <h1 class="text-xl font-bold text-slate-800"><i class="pi pi-car text-sky-500 mr-2"></i> Jadwal Pencarian DUDI</h1>
        <p class="text-sm text-slate-500 mt-1">Atur jadwal pengantaran dan ploting petugas untuk setiap mitra DUDI.</p>
      </div>
  ...
  <div class="flex items-center gap-2 w-full sm:w-auto justify-end">
    ...
    <a href="/admin/pengantaran/download-template" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-all shadow-2xs">
  ...
  <DataTable :value="dudiData.data" responsiveLayout="scroll" class="p-datatable-sm w-full">
    ...
    <Column header="Jadwal & Petugas Pengantaran">
      <template #body="{ data }">
        <div v-if="data.pengantaran" class="whitespace-normal">
          <div class="text-sm font-bold text-sky-700 mb-0.5">{{ data.pengantaran.tanggal_pengantaran }}</div>
          <div class="text-xs text-slate-600"><i class="pi pi-user text-[10px] mr-1 text-slate-400"></i> {{ data.pengantaran.guru?.name }}</div>
        </div>
    ...
    <Column header="Status" class="text-center w-36">
      <template #body="{ data }">
        <select 
          :value="data.pengantaran?.status ?? 'Pending'" 
          ...
          :class="[
            ...
            (data.pengantaran?.status ?? 'Pending') === 'Selesai' ? 'text-emerald-600 border-emerald-200 bg-emerald-50/30' :
            (data.pengantaran?.status ?? 'Progress') === 'Progress' ? 'text-amber-600 border-amber-200 bg-amber-50/30' :
            (data.pengantaran?.status ?? 'Tolak') === 'Tolak' ? 'text-rose-600 border-rose-200 bg-rose-50/30' : 'text-slate-600 border-slate-200 bg-slate-50'
          ]">
    ...
    <Column header="Aksi" class="text-center w-28">
      <template #body="{ data }">
        <div class="flex justify-center gap-1">
          ...
          <Button :icon="data.pengantaran ? 'pi pi-pencil' : 'pi pi-calendar-plus'" :severity="data.pengantaran ? 'info' : 'success'" text class="w-8 h-8 p-0 rounded-lg" @click="modalRef.open(data)" v-tooltip.top="'Atur Jadwal'" />
        </div>
      </template>
    </Column>
  </DataTable>
...
const handleExcelUpload = (event) => {
  ...
  form.post(route('admin.pengantaran.import'), {
    preserveScroll: true,
    onSuccess: () => {
...
const updateStatus = (dudi, newStatus) => {
  router.post(route('admin.pengantaran.store'), {
    dudi_id: dudi.id,
    status: newStatus,
    // Jika data pengantaran belum ada, kirim null/kosong (sudah di-handle nullable di backend)
    guru_id: dudi.pengantaran?.guru_id ?? null,
    tanggal_pengantaran: dudi.pengantaran?.tanggal_pengantaran ?? null
  }, { 
    preserveScroll: true,
    preserveState: true 
  });
...
const doSearch = () => {
  // Gunakan route() agar URL otomatis ter-generate dengan benar
  router.get(route('admin.pengantaran.index'), { search: search.value }, { preserveState: true, preserveScroll: true, replace: true });
};
...
onMounted(async () => {
  try { currentVersion = (await axios.get('/pengantaran/version')).data.version; } catch (e) {}
  versionInterval = setInterval(async () => {
    try {
      const res = await axios.get('/pengantaran/version');
